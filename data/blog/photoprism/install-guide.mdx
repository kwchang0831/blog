---
title: 自己在家架設 Google 相簿 - PhotoPrism
date: 'Tue Dec 07 2021 16:52:03 GMT+0800'
lastmod: 'Wed Jan 05 2022 17:30:00 GMT+0800'
tags: ['中文', 'Ubuntu', 'PhotoPrism']
draft: false
summary: '本篇文章主要紀錄如何從頭到尾架設 PhotoPrism - 類似 Google 相簿 的開源照片管理工具。 主要的功能包含有機器學習的圖片自動分類功能；可偵測顏色、物體、人臉等等資訊，讓使用者可以更快速找到照片。 支援顯示照片與影片也支援顯示相機 Exif 的資訊。 也包含了用世界地圖的方式來整理照片。'
images: ['/static/images/photoprism/install-guide/fig01.webp']
layout: PostLayout
authors: ['kwchang0831']
---
...
![fig01](/static/images/photoprism/install-guide/fig01.webp)

<br />

# 介紹

本篇文章主要紀錄如何從頭到尾架設 PhotoPrism - 類似 Google 相簿 的開源照片管理工具。

主要的功能包含有機器學習的圖片自動分類功能： 可偵測顏色、物體、人臉等等資訊。 讓使用者可以更快速找到照片。  
支援顯示照片與影片也支援顯示相機 Exif 的資訊，也包含了用世界地圖的方式來整理照片。

架設 PhotoPrism，最少只需一台主機即可。

根據[系統需求](https://docs.photoprism.org/getting-started/#system-requirements)，這台主機需求至少 2 核心 與 4GB RAM。  
硬碟空間則是除了安裝 OS 與 包含至少 4GB 的 Swap 交換空間，就是依照個人照片檔案庫大小而定了。

根據[擴展性](https://docs.photoprism.org/getting-started/advanced/scalability/)，目前開發團隊主要測試環境是偏向小型伺服器主機與家用用途，檔案數量不超過 50 萬。

若是有其他需求請查看[官網](https://docs.photoprism.org/)或聯繫該開發團隊。
...
![fig01-01](/static/images/photoprism/install-guide/fig01-01.webp)

我們可以搜尋 `dog` (狗) 試試看。 不過目前標籤與關鍵字搜尋只支援英文。

![fig01-02](/static/images/photoprism/install-guide/fig01-02.webp)

或是到 [https://demo.photoprism.app/labels](https://demo.photoprism.app/labels)，這邊我們可以看到所有已經辨識出來的標籤。

![fig01-03](/static/images/photoprism/install-guide/fig01-03.webp)

剩下的功能就各位自己去嘗試了，我們接下來介紹安裝流程。
...
![fig02](/static/images/photoprism/install-guide/fig02.webp)

由一台無線路由器從中華電信的小烏龜連到網際網路，並獨立出 192.168.0.0/24 的網段。  
該網段裡包含一台桌上型電腦、一台 [TrueNAS](https://www.truenas.com/) 伺服器主機、與一部智慧型手機。

[TrueNAS](https://www.truenas.com/) 伺服器主機設置了 Samba 內網用共享資料夾 與 一個 Virtual Machine (VM) 虛擬電腦 instance 準備用來架設 PhotoPrism。  
而該台 [TrueNAS](https://www.truenas.com/) 伺服器主機使用了 ZFS 檔案系統，並使用 RAID-Z2 的硬碟陣列模式來提高資料的安全性。

桌上型電腦、智慧型手機、與 VM 都可以透過內網存取 [TrueNAS](https://www.truenas.com/) 伺服器主機上的 Samba 內網用共享資料夾，新增與刪除檔案。  
PhotoPrism 的主機只會讀取 Samba 內網用共享資料夾裡的檔案，並不會變更或修改檔案。PhotoPrism 主要是在讀取照片與影片檔案之後，去進行分析整理後生成根據這些資料的資料。

# [選用] 新增 Linux VM 在 TrueNAS

若只是要用單主機安裝 PhotoPrism，可以直接跳過此步驟。

打開 TrueNAS 的 WebAdmin (http://192.168.0.2)。
進入 Virtual Machines 選單並 Add 新增新的 Instance 。

新增一個 有一顆 4 核心雙線程 CPU 與 512GB 硬碟空間 的 VM。

以下為參考流程，

![fig03](/static/images/photoprism/install-guide/fig03.webp)

![fig04](/static/images/photoprism/install-guide/fig04.webp)

![fig05](/static/images/photoprism/install-guide/fig05.webp)

![fig06](/static/images/photoprism/install-guide/fig06.webp)

![fig07](/static/images/photoprism/install-guide/fig07.webp)

![fig08](/static/images/photoprism/install-guide/fig08.webp)

之後點擊新增好的 VM，然後點擊 VNC 連線進去操作安裝過程。

# 安裝 Ubuntu Server

下載 Ubuntu Server 20.04.3 LTS
https://ubuntu.com/download/server

進入 Ubuntu 安裝。

![fig09](/static/images/photoprism/install-guide/fig09.webp)

都是選 English (US) 就可以了。

![fig10](/static/images/photoprism/install-guide/fig10.webp)

網路設定，預設是使用 DHCP 自動取得 IP。

![fig11](/static/images/photoprism/install-guide/fig11.webp)

我們這邊會示範自訂 IP。 上下選到網路介面然後選擇 Edit IPv4。

![fig12](/static/images/photoprism/install-guide/fig12.webp)

IPv4 Method 改成 Manual 手動的方式。  
然後這邊 Subnet 的寫法不是用 Subnet Mask (255.255.255.0)方式來寫。  
根據稍早的示範環境架構圖，我們的無線路由器在 192.168.0.1，整個網段為 192.168.0.0/24。

![fig13](/static/images/photoprism/install-guide/fig13.webp)

Proxy 與 Mirror 的部分都不用更動。

![fig14](/static/images/photoprism/install-guide/fig14.webp)

![fig15](/static/images/photoprism/install-guide/fig15.webp)

設置存儲空間。

![fig16](/static/images/photoprism/install-guide/fig16.webp)

存儲空間這邊建議直接按下 Reset 之後，再自行修改。

![fig17](/static/images/photoprism/install-guide/fig17.webp)

選擇物理硬碟後，新增 GPT Partition。

![fig18](/static/images/photoprism/install-guide/fig18.webp)

我們只需要新增 Mount: / 的空間即可，安裝精靈會幫我們增加開機要用的 Partition (/bott/efi)。  
我是直接輸入超過 max size 的容量，安裝精靈會幫我們修正成可容許的最大空間。

![fig19](/static/images/photoprism/install-guide/fig19.webp)

![fig20](/static/images/photoprism/install-guide/fig20.webp)

按下 Done 就完成設置存儲空間。

![fig21](/static/images/photoprism/install-guide/fig21.webp)

再次確認，選擇繼續 Continue。

![fig22](/static/images/photoprism/install-guide/fig22.webp)

建立使用者檔案。

![fig23](/static/images/photoprism/install-guide/fig23.webp)

這邊要選擇安裝 OpenSSH Server，這樣我們之後才可以從遠端電腦用 SSH 連線進來。

![fig24](/static/images/photoprism/install-guide/fig24.webp)

這邊 Snaps 建議都不用安裝，直接繼續就好。有需要的套件，我們之後會一起安裝。

![fig25](/static/images/photoprism/install-guide/fig25.webp)

開始安裝中。

![fig26](/static/images/photoprism/install-guide/fig26.webp)

安裝完成。

![fig27](/static/images/photoprism/install-guide/fig27.webp)

關機，退出安裝用 USB 或 CD。重開機即可。

# [選用] 在 TrueNAS 的 Linux VM 安裝完成後刪除安裝光碟機

顯示安裝完成，
這時候我們回到 TrueNAS 網頁介面把這 VM 關機 (POWER OFF)。

![fig28](/static/images/photoprism/install-guide/fig28.webp)

點選 DEVICES 把 CDROM (安裝光碟) 的部分刪除。  
這樣我們才能正常開機而不是再次進入安裝程序。

![fig29](/static/images/photoprism/install-guide/fig29.webp)
...
開機後畫面。  
輸入我們安裝時設置的使用者帳號與密碼登入即可。

![fig30](/static/images/photoprism/install-guide/fig30.webp)

這樣就完成了。  
我們之後就可以準備開始安裝 PhotoPrism。

![fig31](/static/images/photoprism/install-guide/fig31.webp)

# [選用] 使用 SSH 進入 Ubuntu Server
...
![fig32](/static/images/photoprism/install-guide/fig32.webp)

![fig33](/static/images/photoprism/install-guide/fig33.webp)

# [選用] 架設 Samba 分享資料夾

如果檔案會儲存在本機上，並透過內網的方式分享給其他電腦設備的話，
我們需要設定架設 Samba 分享資料夾。
...
下載 2.2.2 版本 Docker Compose (若有更新版本可自行修改下方的網址)
...
如果是用 SSH 登入的話，掃描沒有結束記得不要關閉，不然就會被中斷。
如果想要關閉 SSH 然後不中斷處理程序的話，建議使用 [tmux](https://github.com/tmux/tmux)。
...
或是到 https://myaccount.google.com/dashboard  
點選 下載您的資料 。

![fig01-04](/static/images/photoprism/install-guide/fig01-04.webp)

只選擇 Google 相簿後，點選下一步

![fig01-05](/static/images/photoprism/install-guide/fig01-05.webp)

然後依造自己喜好的設定，建立匯出作業。

![fig01-06](/static/images/photoprism/install-guide/fig01-06.webp)
